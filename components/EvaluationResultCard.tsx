...
const EvaluationResultCard: React.FC<EvaluationResultCardProps> = ({ trial, onDelete }) => {
  ...
  const scores = trial.evaluation
    ? [
        normalizeScore(trial.evaluation.accuracy.score),
        normalizeScore(trial.evaluation.relevance.score),
        normalizeScore(trial.evaluation.completeness.score),
        normalizeScore(trial.evaluation.clarity.score),
        normalizeScore(trial.evaluation.safetyAndBias.score),
        normalizeScore(trial.evaluation.hallucinationRisk.score)
      ]
    : [];
            ...
            <div className={`p-10 rounded-[48px] border flex flex-col md:flex-row items-center justify-between ${getVerdictStyle(trial.evaluation!.finalVerdict)} shadow-2xl relative overflow-hidden`}>
               ...
               <div className="mt-8 md:mt-0 flex space-x-6 relative z-10">
                  ...
                  <div className="px-10 py-7 bg-black/40 rounded-3xl border border-white/5 text-center">
                    ...
                    <span className="text-3xl font-[900] tracking-tighter">{overallScore}<span className="text-sm opacity-30 font-bold ml-1">/10</span></span>
                  </div>
               </div>
            </div>
               ...
               <div className="min-h-[400px]">
                  {activeTab === 'metrics' && (
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-6 duration-700">
                      <MetricBlock label="Accuracy" score={normalizeScore(trial.evaluation!.accuracy.score)} justification={trial.evaluation!.accuracy.justification} />
                      <MetricBlock label="Relevance" score={normalizeScore(trial.evaluation!.relevance.score)} justification={trial.evaluation!.relevance.justification} />
                      <MetricBlock label="Completeness" score={normalizeScore(trial.evaluation!.completeness.score)} justification={trial.evaluation!.completeness.justification} />
                      <MetricBlock label="Clarity" score={normalizeScore(trial.evaluation!.clarity.score)} justification={trial.evaluation!.clarity.justification} />
                      <MetricBlock label="Safety" score={normalizeScore(trial.evaluation!.safetyAndBias.score)} justification={trial.evaluation!.safetyAndBias.justification} />
                      <MetricBlock label="Hallucination_Risk" score={normalizeScore(trial.evaluation!.hallucinationRisk.score)} justification={trial.evaluation!.hallucinationRisk.justification} />
